...
import { Calendar as CalendarIcon, Shield, ChevronLeft } from 'lucide-react';
        ...
        <div className="bg-white rounded-[40px] shadow-xl shadow-indigo-100/50 border border-slate-100 p-8 md:p-16 space-y-12">
          ...
          <div className="space-y-10 prose prose-slate prose-indigo max-w-none">
            ...
            <section className="p-8 bg-indigo-50 rounded-3xl border border-indigo-100">
              <h3 className="text-2xl font-bold text-slate-900 mb-4 flex items-center gap-3">
                <span className="w-8 h-8 rounded-lg bg-indigo-600 text-white flex items-center justify-center text-sm font-black">4</span>
                SMS & Text Messaging Opt-In
              </h3>
              <p className="text-slate-700 leading-relaxed font-bold mb-4 italic">
                By opting in via our dedicated Opt-In page, you expressly consent to receive automated messaging (SMS or text messaging) for access codes and booking updates.
              </p>
              <div className="flex flex-col md:flex-row gap-8 items-center bg-white p-6 rounded-2xl border border-indigo-200">
                <QRCodeSVG value={`${window.location.origin}/opt-in`} size={120} />
                <div>
                  <h4 className="font-bold text-slate-900 mb-2">Verification & Consent</h4>
                  <p className="text-sm text-slate-500 mb-4 italic">
                    To satisfy vendor verification requirements, please visit our <button onClick={() => navigate('/opt-in')} className="text-indigo-600 underline font-bold">dedicated Opt-in menu</button> to manage your communication preferences.
                  </p>
                  <a 
                    href={CONSENT_Doc_URL} 
                    target="_blank" 
                    rel="noopener noreferrer"
                    className="inline-block bg-slate-900 text-white px-6 py-2 rounded-xl text-sm font-bold hover:bg-slate-800 transition-colors"
                  >
                    View Legal Consent Document
                  </a>
                </div>
              </div>
            </section>
